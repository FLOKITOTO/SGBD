          ...
          {data.map((item, key) => (
            <tr key={key}>
              ...
              {table.map((tableName, index)=> {
                return (
                  <div key={index} onClick={()=> getDataFomTable(item, tableName)}>{tableName}</div>
                )
              })}
              ...
              <td>
                ...
                <input
                  type="text"
                  defaultValue={tableData.data !== undefined  ? tableData.data[0][0] : ""}
                  onBlur={(e) => {
              ...
              <td>
                <input
                  type="text"
                  defaultValue={tableData.data !== undefined  ? tableData.data[0][1] : ""}
                  onBlur={(e) => {
                    const updatedData = { nom: e.target.value };
                    updateData(item.id, updatedData);
                  }}
              ...
              <td>
                <input
                  type="text"
                  defaultValue={tableData.data !== undefined  ? tableData.data[0][2] : ""}
                  onBlur={(e) => {
                    const updatedData = { nom: e.target.value };
                    updateData(item.id, updatedData);
                  }}
